...
- (void) rateCalculator {
    ...
    NSLog(@"Rx: %li  RxR: %li  Lost:%li", self.rcRxPackets, self.rxRate, self.rcLostPacketCount);
}
...
- (void) streamHandler:(VITA *)vitaPacket {
    ...
    DAXFrame *dax = [[DAXFrame alloc]init];
    ...
    dispatch_async(self.runQueue, ^(void) {
            [self.delegate streamReceive:dax];
    });
...
- (void) setTransmitEnabled:(BOOL)transmitEnabled {
    NSString *cmd = [NSString stringWithFormat:@"dax audio set %i tx=%i",
                     self.daxChannel, self.transmitEnabled];
